...
module.exports = {
    ...
    // Returns the local path for the user config (windows, mac, linux)
	getLocalUserConfig(modules) {
		let userConfig;

		Object.keys(modules).forEach(key => {
			if (userConfig === undefined) {
				try {
					userConfig = require(this.getLocalUserConfigPath(modules[key]));
				} catch (err) {} // Do nothing when the path could not be read
			}
		});
	...
	writeConfig(module, config) {
		// Gets the local path for where to save the config
		const modulePath = this.getLocalUserConfigPath(module);
