            ...
            input.getValidationError = function () {
                try {
                    ...
                    if (value === null) {
                        ...
                        /// JLC
                        if (s.requiredifField && s.requiredifValue) {
                            // var requiredInput = $("[data-schema-id='" + parentSchema.properties[s.requiredIf[key]] + "']");
							var requiredInput = $("[data-title-name='" + s.requiredifField + "']");

							if (requiredInput.is(":checkbox")) {
							    var requiredValue = requiredInput.prop("checked");
							} else {
							    var requiredValue = requiredInput.val();
							}

							if (requiredValue == s.requiredifValue) {
							    return BrutusinForms.messages["required"];
							}
                        }
                        /// end JLC
...
//        }
            ...
            input.id = getInputId();
            // JLC
            input.setAttribute("data-schema-id", s.$id);
            input.setAttribute("data-title-name", s.title);
            // end JLC
            inputCounter++;
        ...
        renderers["object"] = function (container, id, parentObject, propertyProvider, value) {
            ...
            if (s.hasOwnProperty("properties")) {
                ...
                for (var prop in s.properties) {
					...
					// JLC
		            if (propSchema.showifField) td1.setAttribute("data-showif-field", propSchema.showifField);
		            if (propSchema.showifValue) td1.setAttribute("data-showif-value", propSchema.showifValue);
		            if (propSchema.requiredifField) td1.setAttribute("data-requiredif-field", propSchema.requiredifField);
		            if (propSchema.requiredifValue) td1.setAttribute("data-requiredif-value", propSchema.requiredifValue);
					// end JLC
                    var td2 = document.createElement("td");
                    td2.className = "prop-value";
            ...
            if (schema.hasOwnProperty("oneOf")) {
                ...
                if (schema.properties) {
                    ...
                    for (var prop in schema.properties) {
                        ...
                        //// JLC
                        if (schema.showIf) {
							for (var key in schema.showIf) {
								var dependency = schema.showIf[key];

								if (prop == dependency.property) {
									subSchema.showifField = schema.properties[dependency.dependsOn].title;
									subSchema.showifValue = dependency.value;
								}
                            }
                        }
						if (schema.requiredIf) {
							for (var key in schema.requiredIf) {
								var dependency = schema.requiredIf[key];

								if (prop == dependency.property) {
									subSchema.requiredifField = schema.properties[dependency.dependsOn].title;
									subSchema.requiredifValue = dependency.value;
								}
							}
						}
                        // if (schema.requiredIf) {
						// 	for (var key in schema.requiredIf) {
						// 		if (key == prop) {
                        //             subSchema.requiredIf = schema.requiredIf[key];
                        //         }
                        //     }
                        // }
                        ///// end JLC
