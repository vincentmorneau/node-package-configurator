		...
		function receivedText(e) {
			...
			try {
				...
				if ($("[data-project='" + $("#active-project").val() + "']").length > 0) {
					alertify.prompt('Project exists.', 'This project already exists. Please rename it or delete the existing one.', project, function(evt, value) {
				...
				$.ajax({
					...
					success: function(data) {
						if (data.success) {
							...
							$("#original-project, #active-project").val("");
							$(".project-title").text("");
							$(".side-nav").html(data.menu);
							$(".buttons").html("");
						} else {
...
$(document).ready(function() {
	...
	$(document).on('click', '.create', function() {
		alertify.prompt('New project', 'Enter the name for your new project.', $("#active-project").val(), function(evt, value) {
			// Ensures we are not importing a file under the same project name
			preventProjectDuplicates(value, function(){
				npc.get.project(value);
			});
		}, function() {});
	});
